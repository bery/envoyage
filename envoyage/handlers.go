...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	pc.Namespace = os.Getenv("NAMESPACE")

	pc.AwsInstances, err = getAwsInstances()
	if err != nil {
		log.Printf("Unable to get AWS instances: %s", err)
		pc.AwsError = err.Error()
	}

	// render template
	t, err := template.New("base").Funcs(sprig.FuncMap()).Parse(rootPage)
	if err != nil {
		span.RecordError(err)
...
func malwareHandler(w http.ResponseWriter, r *http.Request) {
	...
	if mur := os.Getenv("MALWARE_URL"); mur != "" {
