...
func kubernetesDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "secret":
		if err := cs.CoreV1().Secrets(namespace).Delete(ctx, name, do); err != nil {
			http.Error(w, "Failed deleting secret "+err.Error(), http.StatusBadRequest)
			return
		}

	default:
		http.Error(w, fmt.Sprintf("Unknown resource %s", rt), http.StatusBadRequest)
		return
	}
